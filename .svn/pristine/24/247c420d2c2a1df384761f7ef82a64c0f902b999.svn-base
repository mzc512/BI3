...
function order_dataurlstatis(startdatas,endDates,distdls,distwds,distcywds){
    ...
    $.get("/orderTypeList/method/order/dim/datalist/startDate/" + startDate + "/endDate/" + endDate+"/distdl/"+distdl+"/distwd/"+distwd+"/distcywd/"+distcywd, function (data, status) {
        if(data.result) {
            if ($("#dist_wd option:selected").val() != "全部" && $("#dist_cywd option:selected").val() != "全部") {
                ...
                $('#searchTable').append(
                    ...
                    '<th data-field="amt" data-sortable="true" data-formatter="amtFormatter" >指标</th>' +
                    '</tr>' +
                    '</thead>'
                )
                ...
                option.series[0].data = dldata.reverse();
            ...
            //$('#searchTable').delegate('.dim1ID', "click", function () {
            //    var dim1 = $(this).html();
            //    var dim2 = $(this).html();
            //    var clk1 = "clk1";
            //    if (distwd == "D_D00") {
            ...
            //$('#searchTable').delegate('.dim2ID', "click", function () {
            //    var dim2 = $(this).parent().find('.dim4ID').html();
            //    var dim1 = $(this).parent().find('.dim3ID').html();
            //    var clk2 = "clk2";
            //    order_click_dataall(startDate, endDate, endDate, distdl, distwd, distcywd, dim1, dim2, clk2);
            //});
            //$('#searchTable').delegate('.dim3ID', "click", function () {
            //    var dim1 = $(this).html();
            //    var dim2 = "ALL"
            //    var clk3 = "clk3";
            //    order_click_dataall(startDate, endDate, endDate, distdl, distwd, distcywd, dim1, dim2, clk3);
...
function order_click_dataall(startDate,endDate,endDate,distdl,distwd,distcywd,dim1,dim2,clinum){
    ...
    $.get("/orderTypeList/method/order/dim/clickdata/startDate/" + startDate + "/endDate/" + endDate+"/distdl/"+distdl+"/distwd/"+distwd+"/distcywd/"+distcywd+"/val1/"+dim1+"/val2/"+dim2, function (data, status) {
        if(data.result){
            if(clinum=="clk1"){
                $('#searchTable').append(
                    '<thead>' +
                    ...
                    '<th data-field="dim1" data-sortable="true" data-visible="false">ID</th>' +
                    '<th data-field="amt" data-sortable="true" class="dim4ID" data-formatter="amtFormatter">指标</th>' +
                    '</tr>' +
                    '</thead>'
                )
...
$(document).ready(function () {
    ...
    $.get("/orderTypeList/method/order/dim/dropdownlist", function (data, status) {
        // console.log(data);
        if(data.result)
        {
            ...
            {
                $('#dist_dl').append("<option data-dimcode='"+data.data[0].data[i].dim_code+"'value='" +data.data[0].data[i].dim_name+ "'>" +data.data[0].data[i].dim_name+ "</option>");
            }
            for(var i=0;i<dist_wdlen;i++)
            {
                ss[i]=data.data[1].data[i].dim_name;
                dd[i]=data.data[1].data[i].dim_code;
                $('#dist_wd').append("<option data-dimcode='"+data.data[1].data[i].dim_code+"'value='" +data.data[1].data[i].dim_name+ "'>" +data.data[1].data[i].dim_name+ "</option>");
                $('#dist_cywd').append("<option data-dimcode='"+data.data[1].data[i].dim_code+"'value='" +data.data[1].data[i].dim_name+ "'>" +data.data[1].data[i].dim_name+ "</option>");
            }
