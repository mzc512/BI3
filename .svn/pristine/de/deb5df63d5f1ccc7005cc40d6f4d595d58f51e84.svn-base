...
const newolduserweekly='SELECT  WEEK(Login_Dt)+1 AS Reg_Week, SUM(New_Cnt) AS new_cnt, SUM(Old_Cnt) AS old_cnt FROM csc_anasys.user_new_old_cnt WHERE Login_Dt BETWEEN ? AND ? GROUP BY Reg_Week ORDER BY Reg_Week';
const newoldusermothly='SELECT DATE_FORMAT(Login_Dt,"%Y%m") AS Reg_Month, SUM(new_cnt) AS new_cnt, SUM(old_cnt) AS old_cnt FROM csc_anasys.user_new_old_cnt WHERE YEAR(Login_Dt) BETWEEN ? AND ? GROUP BY Reg_Month ORDER BY Reg_Month';
...
const supplierbuyerweekly='SELECT  WEEK(Login_Dt)+1 AS Reg_Week,  SUM(nor_cnt) AS nor_cnt, SUM(ent_cnt) AS ent_cnt FROM csc_anasys.user_nor_ent_cnt WHERE Login_Dt BETWEEN ? AND ? GROUP BY Reg_Week ORDER BY Reg_Week';
const supplierbuyermothly='SELECT DATE_FORMAT(Login_Dt,"%Y%m") AS Reg_Month,  SUM(nor_cnt) AS nor_cnt, SUM(ent_cnt) AS ent_cnt FROM csc_anasys.user_nor_ent_cnt WHERE YEAR(Login_Dt) BETWEEN ? AND ? GROUP BY Reg_Month ORDER BY Reg_Month';
